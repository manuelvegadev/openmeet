...
| `capture-args.ts` | `webcamCaptureArgs(device)` (avfoundation / v4l2, `null` where webcam is unsupported), `screenCaptureArgs(device)` (avfoundation / gdigrab / x11grab at a fixed 1080p30 I420 output) and `rawVideoPlayerArgs()` for ffplay, plus the size constants. wrtc-free so the CLI's `--test-camera` / `--test-screen` run exactly what a call runs |
