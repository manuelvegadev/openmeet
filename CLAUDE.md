...
- **Media**: WebRTC with stereo Opus audio (48 kHz, 128 kbps per direction by default, RED-protected) and VP8/VP9 video (1080p webcam; screen share in the screen's own aspect ratio, short side ≤ 1080 and long side ≤ 3840 — an ultrawide goes out as 2580x1080 — capped at 2.5 Mbps and scaled down as the mesh grows)
...
| `scripts/resampler-test.ts`, `scripts/channels-test.ts`, `scripts/sdp-test.ts`, `scripts/nvidia-broadcast-test.ts`, `scripts/share-probe.ts`, `scripts/theme-test.ts`, `scripts/chat-test.ts` | Pure, device-free checks: the resampler (SNR per ratio), the channel policy, the Opus negotiation helpers against real wrtc SDP, the NVIDIA Broadcast name matching, the palette (4.5:1 on the background, and every meaningful pair still distinct after chalk's 256-colour rounding), the chat panel (renders the real `ChatLog` through Ink into a fake 60x8 terminal and drives it with key bytes: tail-following, wrapping, ↑/Page Up with the hidden count, a new entry not moving a scrolled view, ↓/Page Down resuming, arrows ignored when the input is unfocused, and `mergeChat`'s ordering), and the screen sender — two real `PeerConnectionManager`s in one process, asserting that a share started mid-call and one present at join both encode (gotcha 33). All run in CI on the three OSes |
...
| Component | Purpose |
|-----------|---------|
| `home-screen.tsx` | Join a room; shows who you are as `[name]` in your colour |
...
| `room-view.tsx` | Main room. Header (version, room, count, `Elapsed`, link stats), then `SplitPanes`: the chat on the left — the conversation and the composer, which brings its own rule — and the participants on the right, with the debug panel under them while debug mode is on. The screen and camera pickers are `Modal`s over it; the audio device change is a `Screen`. `q` twice leaves, drawn in the header — Escape is for cancelling a modal and clearing a draft, never for ending a call |
| `split-panes.tsx` | `SplitPanes`: the room's body, chat pane flexible on the left, people pane at `PARTICIPANTS_WIDTH` on the right, with the `┬`/`┴` rules above and below built from the same two sizes so the junctions land on the divider at any width. The divider is the chat pane's own right border, so a `Rule` inside either pane draws its junction on it (a child may draw over its parent's border; Ink draws the border first) — but only from a box with no horizontal padding, since the rule's one-column bleed and a padding of one cancel out and strand the junctions a cell inside. Shared by the room and `scripts/chat-demo.tsx`, so the demo shows the real layout |
...
| `participant-list.tsx` | The participants column, one line per person: speaking dot, selection marker, `[name]` and state tags on the left, numbers and the 10-cell meter on the right (meter last, so they line up). Tags are single letters run together with no background — `mcs`, mute orange, cam purple, screen blue — so brackets mean a name and nothing else; the case says whether you have that peer's window open (lower while they send, upper while you watch), and the volume shows only when it is not 100%. You come first with no `(you)`, with your own keys under you: a `Divider` inside the padding groups your row with them, and a `Rule` below, outside it, is the section break between you and everyone else. The peer keys at the foot of the pane get a `Divider` too, so the meters' track does not run into the buttons' backgrounds. `PARTICIPANTS_WIDTH` (48) is measured with `string-width` from the widest line it can hold — a name at `NAME_MAX_CELLS` with every tag — so nothing wraps and the chat, the flexible side, never reflows when a tag appears. A peer's name is clamped to the same limit on display, whatever their client sent |
...
| `level-bar.tsx` | `levelColor` (the meter's one threshold ladder), `MicBar` (the mic-test meter, 30 cells) and `VuMeter` (the per-participant one, 10 cells, its track shortened by that peer's volume). Both draw a `Bar` in eighth-blocks (`▏▎▍▌▋▊▉█`) in the level colour over a `theme.surface` track, so a ten-cell bar has 80 steps and a partial cell is the same two colours as its neighbours; `VU_SUBSTEPS`/`VU_LEVEL_STEPS` in `audio/constants.ts` are shared with the engine, which quantizes levels to the same steps before sending. Ballistics are the engine's too: `followLevel` (fast attack, release at `VU_RELEASE_DB_PER_S`, 20 dB/s) runs on every 10 ms frame in `AudioManager` and in the mic test, so a pop rises at once and falls over a second; the TUI draws what it gets |
...
| `settings-view.tsx` | Settings screen: Profile first (the same two `ProfileSetup` steps as the first start, applied on next join), then the device pickers (one `Screen`, a table of three), overlay, channels, noise suppression, bitrates, render pause |
...
| `devices.ts` | Video/screen device enumeration (macOS avfoundation, Linux xrandr, Windows PowerShell `Screen.AllScreens` with DPI awareness). A screen carries the monitor's own name where the OS knows it — on macOS `system_profiler` supplies it ("Odyssey G85SB"), since ffmpeg only ever says "Capture screen 0" — plus its size and whether it is the main display, which is what the picker shows. Screens are cached 60 s and `prefetchScreenDevices()` warms the cache when a room opens, because the PowerShell call costs ~1 s and runs synchronously in the TUI on the `s` key |
...
15. **Terminal settings persistence**: Settings stored at `~/.config/openmeet/settings.json`. Includes `name` and `color` (null until the first-start profile screen sets them, which is how the app knows to show it), audio device IDs, video device ID, overlay toggle, and `devicesConfigured` flag. Legacy flat files (`audio-input`/`audio-output`) auto-migrated on first read.
