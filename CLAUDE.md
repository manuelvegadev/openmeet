...
| `room-view.tsx` | Main room. Header (version, room, count, `Elapsed`, link stats), then `SplitPanes`: the chat on the left — conversation, its rule, its input — and the participants on the right, with the debug panel under them while debug mode is on. The screen picker is a `Modal` over it; the device change is a `Screen` |
...
| `chat-input.tsx` | The chat's input row, under the conversation and as wide as the chat column, not the window |
