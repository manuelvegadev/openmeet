...
| `scripts/resampler-test.ts`, `scripts/channels-test.ts`, `scripts/sdp-test.ts`, `scripts/nvidia-broadcast-test.ts`, `scripts/share-probe.ts` | Pure, device-free checks: the resampler (SNR per ratio), the channel policy, the Opus negotiation helpers against real wrtc SDP, the NVIDIA Broadcast name matching, and the screen sender — two real `PeerConnectionManager`s in one process, asserting that a share started mid-call and one present at join both encode (gotcha 33). All run in CI on the three OSes |
