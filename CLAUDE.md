...
| `status-bar.tsx` | The room's keys, drawn where the thing they act on is, all from `key-hints.tsx`: `MyActions` (`m`/`s`/`v`) under your own row in the participants column, `PeerActions` (`↑↓`/`-/+`/`w`/`e`) pinned under the list, and `RoomBar` (`d`/`o`/`g`) along the bottom — the chat's `tab` chip lives in `chat-input.tsx`, and leaving (`q`, twice) in the header. Every row greys out whole while the chat input has the focus, and `PeerActions` keeps its keys and labels whatever the selected peer is doing, so the list never jumps; whether you already have their window open is in their tags, not in the label |
