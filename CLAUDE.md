...
| `chat-input.tsx` | The chat's row: the prompt, the draft, and the chat's own keys on the right — `tab` to hand the keyboard back and forth, and `esc esc` to throw away a draft rather than holding backspace down a long message. Two presses, armed for 2 s, disarmed by any other key |
