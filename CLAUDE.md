...
| `name.tsx` | `Name`: a participant's name the one way it is ever shown — `[name]`, brackets included, in their colour (hashed from the name when a peer sent none); `column` pads it to `NAME_WIDTH` |
...
| `room-view.tsx` | Main room. Header (version, room, count, `Elapsed`, link stats), then `SplitPanes`: the chat on the left — conversation, its rule, its input — and the participants on the right, with the debug panel under them while debug mode is on. Its sub-screens (screen picker, device change) are `Screen`s |
...
| `participant-list.tsx` | The participants column, one line per person: speaking dot, selection marker, name and tags on the left, numbers and the 10-cell meter on the right (meter last, so they line up). You come first with no `(you)`, set apart by a blank line. A peer's `[cam]`/`[scr]` fold "you have their window open" into their colour (green), and the volume shows only when it is not 100%. `PARTICIPANTS_WIDTH` (64) is measured with `string-width` from the widest line it can hold — a name at `NAME_MAX_CELLS` with every tag — so nothing wraps and the chat, the flexible side, never reflows when a tag appears. A peer's name is clamped to the same limit on display, whatever their client sent |
