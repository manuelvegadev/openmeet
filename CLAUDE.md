...
| `split-panes.tsx` | `SplitPanes`: the room's body, chat pane flexible on the left, people pane at `PARTICIPANTS_WIDTH` on the right, with the `┬`/`┴` rules above and below built from the same two sizes so the junctions land on the divider at any width. The divider is the chat pane's own right border, so a `Rule` inside the chat pane ends in a `┤` on it (a child may draw over its parent's border; Ink draws the border first) and one inside the people pane, a later sibling, starts with a `├` on it. Shared by the room and `scripts/chat-demo.tsx`, so the demo shows the real layout |
